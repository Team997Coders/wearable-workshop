...
def Run(play_wave: bool):
    ...
    if play_wave:
        ...
        sample_buffer = asyncio.run(record_sample(MIC_ADC, SAMPLE_SIZE, sample_rate=SAMPLE_RATE, buffer=None))
        new_buffer = None
        #print(f'Got first sample: {sample_buffer}')
        while True:
            new_buffer_task = asyncio.create_task(record_sample(MIC_ADC, SAMPLE_BITE_SIZE, sample_rate=SAMPLE_RATE, buffer=new_buffer))
            float_array = (sample_buffer - half_buffer_value) / half_buffer_value
            #float_array = float_array / np.max(float_array)
            #print(f'{float_array[0]:0.5f}')
            power_spectrum = ulab.utils.spectrogram(float_array)
            display.show(power_spectrum)
            pixels.show()

            sample_buffer = roll_buffer(sample_buffer, SAMPLE_BITE_SIZE)
            new_buffer = asyncio.run_until_complete(new_buffer_task)
